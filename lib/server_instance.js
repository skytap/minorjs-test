...
extend(ServerInstance.prototype, EventEmitter.prototype, {
  ...
  run : function (test) {
    ...
    runner.on('exit', function (code) {
      if (code !== 0) {
        return self._workerProcessExitedWithError(code)
      }
      self._onDone();
    });
  ...
  _setupListeners : function () {
    ...
    this.server.on('exit', function (code) {
      if (code !== 0) {
        self._workerProcessExitedWithError(code)
      }
    });
        ...
        default: {
          if (!self.runner) {
            if (process.env.TEST_DEBUG === '1') {
              console.log('Tried to send message:"', message, '" to runner, but it was already disconnected.')
            }
            break;
          }
  ...
  _workerProcessExitedWithError : function (code) {
    this.emit('workerProcessExitedWithError', code);
  }
});
