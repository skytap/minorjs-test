...
module.exports = Functional = {
  ...
  start : function (options) {
    ...
    this._loadTestSuites()
      ...
      .then(function () {
        ...
        return [ self._getOpenPorts(), self._getOpenPorts() ];
      })
      .spread(function (ports, webSocketPorts) {
        for (var index in ports) {
          var port          = ports[index],
              webSocketPort = webSocketPorts[index];

          self._createServerWorker(port, webSocketPort);
        }
      })
      .done();
  },
  ...
  /**
   * Functional._createServerWorker(port, webSocketPort)
   * - port (Integer)
   * - webSocketPort (Integer)
   **/
  _createServerWorker : function (port, webSocketPort) {
    var self           = this,
        serverInstance = this._getServerInstance(port, webSocketPort);
  ...
  /**
   * Functional._getServerInstance(port, webSocketPort) -> Object
   * - port (Integer)
   * - webSocketPort (Integer)
   **/
  _getServerInstance : function (port, webSocketPort) {
    return new ServerInstance(port, webSocketPort, this.options.server);
  },
  ...
  _loadTestSuites : function () {
    var deferred = Q.defer();
    glob('test/functional/**/*.{js,coffee}', deferred.makeNodeResolver());
    return deferred.promise;
  },
