...
module.exports = Mock = {
  ...
  _removeOne : function (url, options) {
    ...
    optionsCopy.port     = urlParts.port || ((urlParts.protocol === 'http:') ? 80 : 443);
    optionsCopy.proto    = urlParts.protocol;
    optionsCopy.hostname = urlParts.hostname;
    optionsCopy.method   = optionsCopy.method || 'get';
    optionsCopy.path     = (optionsCopy.query) ? optionsCopy.path + '?' + optionsCopy.query : optionsCopy.path;

    return nock.removeInterceptor(optionsCopy);
  },
