...
function DotFormatter(options) {
  ...
  self.handleScenarioResultEvent = function handleScenarioResultEvent(event, callback) {
    var scenario = event.getPayloadItem('scenarioResult').getScenario();
  ...
  self.storeFailedScenario = function storeFailedScenario(scenario) {
    var name = scenario.name;
    var uri = path.relative(process.cwd(), scenario.uri);
  ...
  self.logScenariosSummary = function logScenariosSummary(featuresResult) {
    self.logCountSummary('scenario', featuresResult.counts);
  };
