...
extend(CucumberReporter.prototype, {
  ...
  hydratePayload: function (eventName, payload) {
    ...
    switch (eventName) {
      ...
      case 'ScenarioResult':
        result = {
          scenarioResult: new Cucumber.Runtime.ScenarioResult(
      ...
      case 'FeaturesResult':
        result = {
          featuresResult: new Cucumber.Runtime.FeaturesResult(
            payload.featuresResult
          )
        };
        result.featuresResult.counts = payload.featuresResult.counts;
        break;
      ...
      case 'Background':
        result = {
          background: new Cucumber.Ast.Background(
            payload.background.keyword,
            payload.background.name,
            payload.background.description,
            payload.background.uri,
            payload.background.line
          )
        };
        for (index in payload.background.steps) {
          var step = payload.background.steps[index];
          result.background.addStep(step);
        }
        break;
